...
EFI_STATUS ExecuteIpmiCmd(UINT64 KcsTimeout, UINT16 KcsPort, VOID* SendBuf, UINT8 SendLength, VOID* ReceiveBuf, UINT8* ReceiveLength)
{
  ...
  if (EFI_ERROR(Status)) {
    Print(L"SendDataToBmcPort Error: %r\n", Status);
    return Status;
  }
  ...
  if (EFI_ERROR(Status)) {
    Print(L"ReceiveBmcDataFromPort Error: %r\n", Status);
    return Status;
  }
...
EFI_STATUS sendBlobCommand(UINT8 BlobCmd, UINT8* Request, UINT8 RequestLength, UINT8* Response, UINT8* ResponseLength)
{
  ...
  FreePool(SendBuf);
  if (EFI_ERROR(Status)) {
    Print(L"Error! ExecuteIpmiCmd returned %r\n", Status);
    FreePool(ReceiveBuf);
    return Status;
  }
#ifdef DEBUG_IPMI_MESSAGES
  ...
  if (  (ReceiveLength < sizeof(BLOB_RESPONSE)) ||
    ...
    Print(L"Error! Response error: Wrong response size %d\n", ReceiveLength);
    FreePool(ReceiveBuf);
    return EFI_DEVICE_ERROR;
  }
  if (((BLOB_RESPONSE*)ReceiveBuf)->Completion != 0x00) {
    Print(L"Error! Response error: Completion byte error\n", ReceiveBuf[0]);
    FreePool(ReceiveBuf);
    return EFI_DEVICE_ERROR;
  }
  if ((((BLOB_RESPONSE*)ReceiveBuf)->OEN[0] != OpenBMC_OEN[0]) ||
    ...
    Print(L"Error! Response error: OEN mismatch\n");
    FreePool(ReceiveBuf);
    return EFI_DEVICE_ERROR;
  }
  if (ReceiveLength >= sizeof(BLOB_RESPONSE_WITH_DATA)) {
    *ResponseLength = ReceiveLength - (sizeof(BLOB_RESPONSE_WITH_DATA) - 1);
    ...
    if (Crc != ((BLOB_RESPONSE_WITH_DATA*)ReceiveBuf)->Crc) {
      Print(L"Error! Response error: CRC mismatch\n");
      FreePool(ReceiveBuf);
      return EFI_DEVICE_ERROR;
    }
...
EFI_STATUS BlobLongWrite(UINT16 Session, UINT32 Offset, UINT8* Data, UINT32 DataSize)
{
  ...
  while (Count < DataSize) {
    ...
    if (EFI_ERROR(Status)) {
      Print(L"Error! BlobWrite failed: %r\n", Status);
      return Status;
    }
    Count += Size;
  }
...
EFI_STATUS BlobSendPCIeRootBridgeDevices(EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL* PciRootBridgeIo, UINT16 Session, UINT32* Offset)
{
  ...
  if (EFI_ERROR(Status)) {
    Print(L"Error! PciRootBridgeIo->Configuration returned: %r\n", Status);
    return Status;
  }
      ...
      for (UINT8 Bus = (UINT8)(AddressDescriptor->AddrRangeMin); Bus <= (UINT8)(AddressDescriptor->AddrRangeMax); Bus++) {
        for (UINT8 Device = 0; Device <= PCI_MAX_DEVICE; Device++) {
          for (UINT8 Func = 0; Func <= PCI_MAX_FUNC; Func++) {
            ...
            if (EFI_ERROR(Status)) {
              Print(L"Error! Failed to use PciRootBridgeIo->Pci.Read: %r\n", Status);
              return Status;
            }
...
EFI_STATUS BlobSendPCIeDevices(EFI_HANDLE ImageHandle, UINT16 Session)
{
  ...
  if (EFI_ERROR(Status)) {
    Print(L"Error! Can't locate gEfiPciRootBridgeIoProtocolGuid: %r\n", Status);
    return Status;
  }
  ...
  for (UINTN Index = 0; Index < HandleCount; Index++) {
    ...
    if (EFI_ERROR(Status)) {
      Print(L"Error! Can't open gEfiPciRootBridgeIoProtocolGuid: %r\n", Status);
      return Status;
    }

    Status = BlobSendPCIeRootBridgeDevices(PciRootBridgeIo, Session, &Offset);
    if (EFI_ERROR(Status)) {
      Print(L"Error! BlobSendPCIeRootBridgeDevices failed with error: %r\n", Status);
      return Status;
    }
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    Print(L"Error! BlobGetCount failed: %r\n", Status);
    return Status;
  }
  Print(L"BlobCount = %d\n", BlobCount);
  ...
  for (UINT32 id = 0; id < BlobCount; id++) {
    ...
    if (EFI_ERROR(Status)) {
      Print(L"Error! BlobEnumerate failed: %r\n", Status);
      FreePool(BlobId);
      return Status;
    }
    ...
    Print(L"PCIe blob is found\n");
    UINT16 Session;
    Print(L"BlobOpen\n");
    Status = BlobOpen(BLOB_STATE_FLAG_OPEN_WRITE, BlobId, &Session);
    FreePool(BlobId);
    if (EFI_ERROR(Status)) {
      Print(L"Error! BlobOpen failed: %r\n", Status);
      return Status;
    }
    Print(L"Session = 0x%04x\n", Session);

    
    Print(L"BlobSendPCIeDevices\n");
    Status = BlobSendPCIeDevices(ImageHandle, Session);
    if (EFI_ERROR(Status)) {
      Print(L"Error! BlobSendPCIeDevices failed: %r\n", Status);
      BlobClose(Session);
      return Status;
    }
    ...
    if (EFI_ERROR(Status)) {
      Print(L"Error! BlobCommit failed: %r\n", Status);
      BlobClose(Session);
      return Status;
    }
    Print(L"BlobClose\n");
    ...
    if (EFI_ERROR(Status)) {
      Print(L"Error! BlobClose failed: %r\n", Status);
      return Status;
    }
    Print(L"PCIe device information was successfully transmitted to the BMC\n");
  }
